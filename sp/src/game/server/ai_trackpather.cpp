...
{
	//Msg("CAI_TrackPather::InitPathingData\n");
...
{
	//Msg("CAI_TrackPather::EnableLeading\n");
...
{
	//Msg("CAI_TrackPather::NextAlongCurrentPath\n");
...
{
	//Msg("CAI_TrackPather::PreviousAlongCurrentPath\n");
...
{
	//Msg("CAI_TrackPather::AdjustForMovementDirection\n");
...
{
	//Msg("CAI_TrackPather::FindTrackBlocker\n");
	trace_t	tr;
...
{
	//Msg("CAI_TrackPather::BestPointOnPath\n");
	...
	for ( int i = 0; i < 2; ++i )
	{
		...
		{
			//Msg("Iterating on path_track %s\n", pTravPath->GetEntityName().ToCStr());
			// Circular loop checking
			if (pTravPath->HasBeenVisited())
			{
				//Msg("path_track %s HasBeenVisited\n\n", pTravPath->GetEntityName().ToCStr());
				break;
			}
			pTravPath->Visit();
			...
			//Msg("Distance between path_track %s (%0.1f %0.1f %0.1f) and targetPos (%0.1f %0.1f %0.1f) is %0.1f\n", pTravPath->GetEntityName().ToCStr(), pTravPath->GetAbsOrigin().x, pTravPath->GetAbsOrigin().y, pTravPath->GetAbsOrigin().z,
			//	targetPos.x, targetPos.y, targetPos.z, flPathDist);
			//Msg("bFarthestPoint is %s\n", bFarthestPoint ? "TRUE" : "FALSE");
			// See if it's closer and it's also not within our avoidance radius
			if ( bFarthestPoint )
			{
				//Msg("Is flPathDist (%0.1f) <= flNearestDist (%0.1f) AND is flNearestDist (%0.1f) <= flFarthestDistSqr (%0.1f)\n", flPathDist, flNearestDist, flNearestDist, flFarthestDistSqr);
				if ((flPathDist <= flNearestDist) && (flNearestDist <= flFarthestDistSqr))
				{
					//Msg("Yes, bad path_track\n\n");
					continue;
				}
			}
			else
			{
				//Msg("Is flPathDist (%0.1f) >= flNearestDist (%0.1f)\n", flPathDist, flNearestDist);
			...
			//Msg("Is flAvoidRadius (%0.1f) non-zero AND is PathDistance2DSqr (%0.1f) <= it?\n", flAvoidRadius, (pTravPath->GetAbsOrigin() - targetPos).Length2DSqr());
			...
			if ( visible )
			{
				...
				//Msg("(%s) && (%s || %s)\n", (pBlocker != NULL) ? "TRUE" : "FALSE", !bHitTarget ? "TRUE" : "FALSE", m_bForcedMove ? "TRUE" : "FALSE");
				if ((pBlocker != NULL) && (!bHitTarget || m_bForcedMove))
				{
					//Msg("%s failed vis check\n\n", pTravPath->GetEntityName().ToCStr());
					continue;
				}
			}
			...
			//Msg("path_track %s is new best, flNearestDist = %0.1f\n\n", pTravPath->GetEntityName().ToCStr(), flNearestDist);
		}
	}
	if (pNearestPath)
	{
		//Msg("pNearestPath exists, %s\n", pNearestPath->GetEntityName().ToCStr());
	}
	else
	{
		//Msg("pNearestPath doesn't exist!!!!\n");
	}
	return pNearestPath;
}
...
{
	//Msg("CAI_TrackPather::ComputeLeadingPointAlongPath\n");
...
{
	//Msg("CAI_TrackPather::ComputeDistanceAlongPathToPoint\n");
...
{
	//Msg("CAI_TrackPather::VisualizeDebugInfo\n");
...
{
	//Msg("CAI_TrackPather::HasLOSToTarget\n");
	CBaseEntity *pTargetEnt = GetTrackPatherTargetEnt();
	if ( !pTargetEnt )
		return true;
...
{
	//Msg("CAI_TrackPather::UpdateCurrentTarget\n");
...
{
	//Msg("CAI_TrackPather::ComputeDistanceToLeadingPosition\n");
...
{
	//Msg("CAI_TrackPather::ComputeDistanceToTargetPosition\n");
...
{
	//Msg("CAI_TrackPather::ComputePathDirection\n");
...
{
	//Msg("CAI_TrackPather::CurrentPathDirection\n");
	if ( m_pCurrentPathTarget )
	{
...
{
	//Msg("CAI_TrackPather::ComputePointAlongCurrentPath\n");
	Vector vecPathDir;
...
{
	//Msg("CAI_TrackPather::ComputePerpDistanceFromPath\n");
...
{
	//Msg("CAI_TrackPather::ComputePointFromPerpDistance\n");
	Vector vecAcross;
	CrossProduct( vecPathDir, Vector( 0, 0, 1 ), vecAcross );
...
{
	//Msg("CAI_TrackPather::FindClosestPointOnPath\n");
	// Find the node nearest to the destination path target if a path is not specified
	if ( pPath == NULL )
	{
		pPath = m_pDestPathTarget;
...
{
	//Msg("CAI_TrackPather::InputStartBreakableMovement\n");
	m_bPatrolBreakable = true;
}

void CAI_TrackPather::InputStopBreakableMovement( inputdata_t &inputdata )
{
	//Msg("CAI_TrackPather::InputStopBreakableMovement\n");
	m_bPatrolBreakable = false;
}
...
{
	//Msg("CAI_TrackPather::InputStartPatrol\n");
	m_bPatrolling = true;	
}
...
{
	//Msg("CAI_TrackPather::InputStopPatrol\n");
	m_bPatrolling = false;	
}
...
{
	//Msg("CAI_TrackPather::InputStartPatrolBreakable\n");
	m_bPatrolBreakable = true;
	m_bPatrolling = true;	
}
...
{
	//Msg("CAI_TrackPather::InputStartLeading\n");
	EnableLeading( true );
...
{
	//Msg("CAI_TrackPather::InputStopLeading\n");
	EnableLeading( false );
}
...
{
	//Msg("CAI_TrackPather::SelectNewDestTarget\n");
	if ( !m_bPatrolling )
		return;
...
{
	//Msg("CAI_TrackPather::UpdateTargetPositionLeading\n");
	Vector targetPos;
	if ( !GetTrackPatherTarget( &targetPos ) )
		return;
...
{
	//Msg("CAI_TrackPather::UpdateTargetPosition\n");
...
{
	//Msg("CAI_TrackPather::TargetSpeedAlongPath\n");
	if ( !GetEnemy() || !IsLeading() )
		return 0.0f;

	Vector vecSmoothedVelocity = GetEnemy()->GetSmoothedVelocity();
	//Msg("TargetSpeedAlongPath: %0.1f\n", DotProduct(vecSmoothedVelocity, TargetPathDirection()));
...
{
	//Msg("CAI_TrackPather::TargetSpeedAcrossPath\n");
	if ( !GetEnemy() || !IsLeading() )
		return 0.0f;

	Vector vecSmoothedVelocity = GetEnemy()->GetSmoothedVelocity();
	//Msg("TargetSpeedAcrossPath: %0.1f\n", DotProduct(vecSmoothedVelocity, TargetPathAcrossDirection()));
...
{
	//Msg("CAI_TrackPather::MaxDistanceFromCurrentPath\n");
...
{
	//Msg("CAI_TrackPather::UpdateTrackNavigation\n");
...
{
	//Msg("CAI_TrackPather::SetFarthestPathDist\n");
...
{
	//Msg("CAI_TrackPather::SetupNewCurrentTarget\n");
	Assert( pTrack );
...
{
	//Msg("CAI_TrackPather::MoveToTrackPoint\n");
...
{
	//Msg("CAI_TrackPather::MoveToClosestTrackPoint\n");
	if ( IsOnSameTrack( pTrack, m_pDestPathTarget ) )
		return;
...
{
	//Msg("CAI_TrackPather::IsOnSameTrack\n");
	if ( pPath1 == pPath2 )
		return true;
...
{
	//Msg("CAI_TrackPather::Teleported\n");
...
{
	//Msg("CAI_TrackPather::ComputePathDistance\n");
	float flDist = 0.0f;
...
{
	//Msg("CAI_TrackPather::IsForwardAlongPath\n");
...
{
	//Msg("CAI_TrackPather::ClosestPointToCurrentPath\n");
	if (!m_pCurrentPathTarget)
	{
...
{
	//Msg("CAI_TrackPather::ComputePathTangent\n");
...
{
	//Msg("CAI_TrackPather::ComputeNormalizedDestVelocity\n");
...
{
	//Msg("CAI_TrackPather::InputSetTrack\n");
...
{
	//Msg("CAI_TrackPather::FlyToPathTrack\n");
	CBaseEntity *pGoalEnt = gEntList.FindEntityByName( NULL, strTrackName );
	if ( pGoalEnt == NULL )
	{
		DevWarning( "%s: Could not find path_track '%s'!\n", GetClassname(), STRING( strTrackName ) );
...
{
	//Msg("CAI_TrackPather::InputFlyToPathTrack\n");
	// Find our specified target
	string_t strTrackName = MAKE_STRING( inputdata.value.String() );
	m_nPauseState = PAUSE_NO_PAUSE;
