...
bool CFuncTank::OnControls( CBaseEntity *pTest )
{
	...
	if (m_hControlVolume && m_hControlVolume->IsTouching(pTest))
		return true;
...
void CFuncTank::Use( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
	...
	else if ( m_hController != pPlayer && useType != USE_OFF )
	{
		...
		if (m_hControlVolume && !m_hControlVolume->IsTouching(pPlayer))
			return;
...
void CFuncTankRocket::Fire( int bulletCount, const Vector &barrelEnd, const Vector &forward, CBaseEntity *pAttacker, bool bIgnoreSpread )
{
	...
	if (HasController())
		pOwner = GetController();
	CMissile *pRocket = (CMissile *)CBaseEntity::Create("rpg_missile", barrelEnd, GetAbsAngles(), pOwner);
...
public:
	...
	static CMortarShell *Create( const Vector &vecStart, const Vector &vecTarget, const Vector &vecShotDir, float flImpactDelay, float flWarnDelay, string_t warnSound, CBaseEntity *pShooter);
...
CMortarShell *CMortarShell::Create(const Vector &vecStart, const Vector &vecTarget, const Vector &vecShotDir, float flImpactDelay, float flWarnDelay, string_t warnSound, CBaseEntity *pShooter)
{
	...
	pShell->Spawn();
	pShell->SetOwnerEntity(pShooter);
...
BEGIN_DATADESC( CFuncTankCombineCannon )
	...
	DEFINE_FIELD(m_bManual, FIELD_BOOLEAN),
	...
	DEFINE_KEYFIELD(m_bManual, FIELD_BOOLEAN, "manual"),
...
void CFuncTankCombineCannon::FuncTankPostThink()
{
	...
	if( m_hTarget.Get() == NULL && !m_bManual)//player-controlled tank fires howver it pleases
	{
...
void CFuncTankCombineCannon::Fire( int bulletCount, const Vector &barrelEnd, const Vector &forward, CBaseEntity *pAttacker, bool bIgnoreSpread )
{
	...
	if( HasSpawnFlags(SF_TANK_AIM_AT_POS) && !m_bManual)
		return;
