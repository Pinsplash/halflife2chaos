...
You can use this stanza to try to counterplace the constraint on the player's head so he gets hauled sideways to the right place on the barnacle, but it is better to just move the tongue before attachment.
	...
	//TODO: not all entities can ragdoll. i would like to make them all ragdollable
	if (m_hRagdoll)
	...
	ragdoll_t *pRagdoll = NULL;
	if (m_hRagdoll)
		pRagdoll = m_hRagdoll->GetRagdoll();
	...
	if (npc_barnacle_swallow.GetBool() && m_hRagdoll)
	{
...
void CNPC_Barnacle::BitePrey( void )
{
	...
	if ( enemyClass == CLASS_ZOMBIE )
	{
		...
		CBaseEntity *pEnemy = GetEnemy();
		int iBloodColor = pEnemy->BloodColor();
		UTIL_BloodSpray(vecBloodPos, Vector(0, 0, -1), iBloodColor, 8, FX_BLOODSPRAY_ALL);
