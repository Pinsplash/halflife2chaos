...
int CCollisionEvent::ShouldSolvePenetration( IPhysicsObject *pObj0, IPhysicsObject *pObj1, void *pGameData0, void *pGameData1, float dt )
{
	...
	if (pEntity0 == NULL || pEntity1 == NULL)
		return true;//technically it was resolved
...
CON_COMMAND_F(surfaceprop, "Reports the surface properties at the cursor", FCVAR_NONE)
{
	if ( !UTIL_IsCommandIssuedByServerAdmin() )
		return;
...
#ifdef PORTAL
ConVar sv_fullsyncclones("sv_fullsyncclones", "1", FCVAR_NONE );
...
static ConCommand air_density("air_density", CC_AirDensity, "Changes the density of air for drag computations.", FCVAR_NONE);
