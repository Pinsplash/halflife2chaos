...
CON_COMMAND_F( achievement_reset_all, "Clears all achievements", FCVAR_NONE )
{
	CAchievementMgr *pAchievementMgr = dynamic_cast<CAchievementMgr *>( engine->GetAchievementMgr() );
	if ( !pAchievementMgr )
		return;
...
CON_COMMAND_F(achievement_reset, "<internal name> Clears specified achievement", FCVAR_NONE)
{
	CAchievementMgr *pAchievementMgr = dynamic_cast<CAchievementMgr *>( engine->GetAchievementMgr() );
	if ( !pAchievementMgr )
		return;
...
CON_COMMAND_F(achievement_status, "Shows status of all achievement", FCVAR_NONE)
{
	CAchievementMgr *pAchievementMgr = dynamic_cast<CAchievementMgr *>( engine->GetAchievementMgr() );
	if ( !pAchievementMgr )
		return;
...
CON_COMMAND_F(achievement_unlock, "<internal name> Unlocks achievement", FCVAR_NONE)
{
	CAchievementMgr *pAchievementMgr = dynamic_cast<CAchievementMgr *>( engine->GetAchievementMgr() );
	if ( !pAchievementMgr )
		return;
...
CON_COMMAND_F(achievement_unlock_all, "Unlocks all achievements", FCVAR_NONE)
{
	CAchievementMgr *pAchievementMgr = dynamic_cast<CAchievementMgr *>( engine->GetAchievementMgr() );
	if ( !pAchievementMgr )
		return;
...
CON_COMMAND_F(achievement_evaluate, "<internal name> Causes failable achievement to be evaluated", FCVAR_NONE)
{
	CAchievementMgr *pAchievementMgr = dynamic_cast<CAchievementMgr *>( engine->GetAchievementMgr() );
	if ( !pAchievementMgr )
		return;
...
CON_COMMAND_F(achievement_test_friend_count, "Counts the # of teammates on local player's friends list", FCVAR_NONE)
{
	CAchievementMgr *pAchievementMgr = dynamic_cast<CAchievementMgr *>( engine->GetAchievementMgr() );
	if ( !pAchievementMgr )
		return;
...
CON_COMMAND_F(achievement_test_clan_count, "Determines if specified # of teammates belong to same clan w/local player", FCVAR_NONE)
{
	CAchievementMgr *pAchievementMgr = dynamic_cast<CAchievementMgr *>( engine->GetAchievementMgr() );
	if ( !pAchievementMgr )
		return;
...
CON_COMMAND_F(achievement_mark_dirty, "Mark achievement data as dirty", FCVAR_NONE)
{
	CAchievementMgr *pAchievementMgr = dynamic_cast<CAchievementMgr *>( engine->GetAchievementMgr() );
	if ( !pAchievementMgr )
		return;
